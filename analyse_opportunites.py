...
# Fonction : Récupération des données essentielles
...
# Réduction des données pour GPT
def extract_relevant_data(cryptos):
    return [
        {
            "name": c["name"],
            "symbol": c["symbol"],
        ...
        for c in cryptos if c["market_cap_rank"] > 10
    ][:20]

# Analyse GPT-4
def generate_ai_analysis(filtered_cryptos):
    prompt = f"""
...
try:
    ...
    filtered_data = extract_relevant_data(cryptos)
    ai = generate_ai_analysis(filtered_data)
