...
Persistence for the new tables introduced by the auto-trading overhaul:
    - auto_sessions
    - auto_decisions
    - trade_groups
    - sms_notifications

Sits alongside ``order_repository`` and shares the same async session factory.
"""
...
class AutoRepository:
    """All Postgres access for the auto-trading stack."""
    ...
    async def upsert_session(self, session_state: Dict[str, Any]) -> None:
        """Insert or update the auto_sessions row for the given user."""
    ...
    async def get_session(self, user_id: str) -> Optional[AutoSessionRow]:
        try:
            db = await self._session()
            try:
                row = await db.get(AutoSessionRow, user_id)
                return row
            finally:
                await db.close()
        except SQLAlchemyError as exc:
            logger.warning("get_session failed for %s: %s", user_id, exc)
            return None

    async def list_enabled_sessions(self) -> List[AutoSessionRow]:
        try:
            db = await self._session()
            try:
                stmt = select(AutoSessionRow).where(AutoSessionRow.enabled.is_(True))
                result = await db.execute(stmt)
                return list(result.scalars())
            finally:
                await db.close()
        except SQLAlchemyError as exc:
            logger.warning("list_enabled_sessions failed: %s", exc)
            return []
    ...
    async def insert_trade_group(self, group: Dict[str, Any]) -> None:
        try:
            ...
            logger.warning(
                "insert_trade_group failed for %s: %s", group.get("id"), exc
            )
    ...
    async def update_trade_group(
        ...
        try:
            ...
            logger.warning("update_trade_group failed for %s: %s", group_id, exc)
    ...
    async def find_open_group(
        ...
        """FIFO lookup — oldest open group on this symbol for this user."""
        try:
            db = await self._session()
            try:
                stmt = (
    ...
    async def groups_needing_backfill(self, limit: int = 100) -> List[TradeGroupRow]:
        """Closed groups whose ``max_drawdown_during_hold`` is still NULL."""
        try:
            db = await self._session()
            try:
                stmt = (
    ...
    async def insert_sms_notification(self, record: Dict[str, Any]) -> None:
        try:
            db = await self._session()
            try:
                row = SmsNotificationRow(**record)
                db.add(row)
                await db.commit()
            finally:
                await db.close()
        except SQLAlchemyError as exc:
            logger.warning("insert_sms_notification failed: %s", exc)
    ...
    async def update_sms_status(
        ...
        *,
        ...
        try:
            db = await self._session()
            try:
                row = await db.get(SmsNotificationRow, sms_id)
                if row is None:
                    return
                row.status = status
            ...
            logger.warning("update_sms_status failed: %s", exc)
